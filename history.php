					...
					<div class="row">
						...
						<div class="col-12 col-lg-9 mt-4 mt-lg-0">
							<div class="card">
                                ...
                                <thead>
                                    <tr>
                                        <th class="product-col">Transaction Id</th>
                                        <!-- <th>Name</th> -->
                                        <th>Bank </th>
                                        <th>Account Name</th>
                                ...
                                <input type="hidden" id="code" value="<?php echo $_SESSION['useruser']; ?>">
                                <tbody id="historysd">
                                <?php
                                $uid= $conn->real_escape_string($_SESSION['userid']);
                                $output='';                                
                                $sqls = $conn->query("SELECT * FROM payout WHERE userid='$uid' ORDER BY id DESC");
                                $total_records =$sqls->num_rows;
                                $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                $start_from = ($page_no - 1) * $total_records_per_page;
                                $s=$conn->query("SELECT * FROM payout WHERE userid='$uid' ORDER BY id DESC LIMIT $start_from, $total_records_per_page");
                                if($s->num_rows>0){
