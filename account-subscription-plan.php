									...
									<div class="table-responsive-sm">
										<table> 
                                            ...
                                            <?php
                                                ...
                                                $sqls = $conn->query("SELECT * FROM `dsub` WHERE userid='$id' ORDER BY id DESC");
                                                $total_records =$sqls->num_rows;
                                                $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                                $start_from = ($page_no - 1) * $total_records_per_page;
                                                
                                                $poo = $conn->query("SELECT * FROM `dsub` WHERE userid='$id' ORDER BY id DESC LIMIT $start_from, $total_records_per_page");
                                                if($poo->num_rows>0){
                                                    while($row = $poo->fetch_assoc()):
                                                ?>
												<tr>
													<td><?php echo $row['pname']; ?></td>
													<td>&#8358;<?php echo number_format($row['dtotal']); ?></td>

													<td>
                                                    &#8358;<?php if($row['amt_paid'] > $row['dtotal']){ echo number_format($row['dtotal']); }else{ echo number_format($row['amt_paid']); } ?>
                                                    
                                                    </td>
                                                    
													<td> &#8358;<?php if($row['dbalance'] <= 0){echo 0; }else{  echo number_format($row['dbalance']); }?></td>
													<td><?php echo ucfirst($row['pstatus']); ?></td>
													<td>
                                                    ...
                                                    Action
                                                        ...
                                                        <div class="dropdown-menu">
                                                            <a class="dropdown-item" href="subscription-payment?transaction_id=<?php echo $row['subid']; ?>">View</a>
                                                                <a class="dropdown-item" href="subscription-payment?transaction_id=<?php echo $row['subid']; ?>">Make Payment</a>
                                                           
                                                            <a class="dropdown-item" href="account-payment-history?transaction_id=<?php echo $row['subid']; ?>">Payment History</a>
                                                            <?php if($row['amt_paid'] == 0 AND $row['pstatus'] != 'cancelled' ){ ?>
                                                            <a class="dropdown-item" id="sub-cancelled" subid="<?php echo $row['subid']; ?>" href="#">Cancel Subscription</a>
