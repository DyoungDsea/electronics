...
.sweep,.sweep2{
 ...
 <div class="card mb-3">
          <div class="card-header">
            ...
            Manage Pending  Duties <span class="text-danger"><?php if(isset($_GET['page_no']) AND @$_GET['page_no'] !=0){ echo "(".$_GET['page_no'].")";} ?></span></div>
          <div class="card-body">
            <div class="table-responsive"  style="min-height:200px">
              <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                ...
                $sqls = $conn->query("SELECT * FROM login INNER JOIN dcart ON dcart.userid=login.userid WHERE dcart.dorder_status='confirmed' AND dcart.dagent_id IS NOT NULL OR dcart.dorder_status='dispatched' AND dcart.dagent_id IS NOT NULL ORDER BY dcart.id DESC ");

                $total_records =$sqls->num_rows;
                $total_no_of_pages = ceil($total_records / $total_records_per_page);
                $start_from = ($page_no - 1) * $total_records_per_page;

                $sky =$conn->query("SELECT * FROM login INNER JOIN dcart ON dcart.userid=login.userid WHERE dcart.dorder_status='confirmed' AND dcart.dagent_id IS NOT NULL OR dcart.dorder_status='dispatched' AND dcart.dagent_id IS NOT NULL ORDER BY dcart.id DESC LIMIT $start_from, $total_records_per_page");
                }
