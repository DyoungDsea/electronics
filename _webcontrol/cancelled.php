            ...
            <div class="table-responsive"  style="min-height:200px">
              <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                ...
                if($r['drank']=="seller"){
                  ...
                  if(isset($_GET['pro_name'])){
                    $orderId = $conn->real_escape_string($_GET['pro_name']);
                    $sqls = $conn->query("SELECT * FROM dcart INNER JOIN login ON dcart.userid=login.userid WHERE dcart.orderid LIKE '%$orderId%' AND dcart.dorder_status='cancelled' AND dagent_id='$userid'  OR login.dname LIKE '%$orderId%' AND dcart.dorder_status='cancelled'  AND dagent_id='$userid'  ORDER BY dcart.id DESC ");
                    $total_records =$sqls->num_rows;
                    $total_no_of_pages = ceil($total_records / $total_records_per_page);
                    $start_from = ($page_no - 1) * $total_records_per_page;
    
                    $sky =$conn->query("SELECT * FROM dcart INNER JOIN login ON dcart.userid=login.userid WHERE dcart.orderid LIKE '%$orderId%' AND dcart.dorder_status='cancelled' AND dagent_id='$userid'  OR login.dname LIKE '%$orderId%' AND dcart.dorder_status='cancelled' AND dagent_id='$userid' OR dcart.dcompany LIKE '%$orderId%' AND dcart.dorder_status='cancelled' AND dagent_id='$userid'  ORDER BY dcart.id DESC  LIMIT $start_from, $total_records_per_page");
    
                    }else{
    
    
                    $sqls = $conn->query("SELECT * FROM dcart INNER JOIN login ON dcart.userid=login.userid WHERE  dcart.dorder_status='cancelled' AND dcart.dagent_id='$userid'  ORDER BY dcart.id DESC ");
    
                    $total_records =$sqls->num_rows;
                    $total_no_of_pages = ceil($total_records / $total_records_per_page);
                    $start_from = ($page_no - 1) * $total_records_per_page;
    
                    $sky =$conn->query("SELECT * FROM dcart INNER JOIN login ON dcart.userid=login.userid WHERE dcart.dorder_status='cancelled' AND dcart.dagent_id='$userid' ORDER BY dcart.id DESC LIMIT $start_from, $total_records_per_page");
                    }
                ...
                if(isset($_GET['pro_name'])){
                $orderId = $conn->real_escape_string($_GET['pro_name']);
                $sqls = $conn->query("SELECT * FROM dcart WHERE orderid LIKE '%$orderId%' AND dorder_status='cancelled' ORDER BY id DESC");
                $total_records =$sqls->num_rows;
                $total_no_of_pages = ceil($total_records / $total_records_per_page);
                $start_from = ($page_no - 1) * $total_records_per_page;

                $sky =$conn->query("SELECT * FROM dcart WHERE orderid LIKE '%$orderId%' AND dorder_status='cancelled' ORDER BY id DESC LIMIT $start_from, $total_records_per_page");
