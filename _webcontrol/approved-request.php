           ...
           <div class="card ">
               ...
               <table class="table table-bordereds">
                                        ...
                                        <tbody>
                                            ...
                                            $sqls =$conn->query("SELECT * FROM dwithdrawal WHERE dstatus='approved' AND dname='$search' OR dstatus='approved' AND damount='$search' OR dstatus='approved' AND ddate='$search' ORDER BY id ASC");
                                            $total_records =$sqls->num_rows;
                                            $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                            $start_from = ($page_no - 1) * $total_records_per_page;

                                            $cup = $conn->query("SELECT * FROM dwithdrawal WHERE dstatus='approved' AND dname='$search' OR dstatus='approved' AND damount='$search' OR dstatus='approved' AND ddate='$search' ORDER BY id ASC LIMIT $start_from, $total_records_per_page");
                                            }else{
                                                $sqls =$conn->query("SELECT * FROM dwithdrawal WHERE dstatus='approved' ORDER BY id ASC");
                                            $total_records =$sqls->num_rows;
                                            $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                            $start_from = ($page_no - 1) * $total_records_per_page;

                                            $cup = $conn->query("SELECT * FROM dwithdrawal WHERE dstatus='approved' ORDER BY id ASC LIMIT $start_from, $total_records_per_page");
                                            }
                                            if($cup->num_rows>0){
                                            ...
                                            <tr>
                                                ...
                                                <div class="btn-group" >
                                                    ...
                                                    <li><a class=" "id="paid" amount='<?php echo $car['damount']; ?>' user='<?php echo $car['userid']; ?>' vid="<?php echo $car['dwithid']; ?>" href="#"><i class="fa fa-check  text-primary"></i> Mark As Paid</a></li>
                                                    <div class="divider"></div>
                                                    <hr>
                                                
                                                <li><a class=" " id="cancelx" amount='<?php echo $car['damount']; ?>' user='<?php echo $car['userid']; ?>' vid="<?php echo $car['dwithid']; ?>" img="" href="#"><i class="fa fa-times  text-primary"></i> Cancel </a></li>
