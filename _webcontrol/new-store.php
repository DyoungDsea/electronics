...
.sweep,.sweep2{
      ...
      <form method="post" action="category_filter" id="tarp">
               <div class="col-md-6" style="float:left;">
                 <div class="form-group">
                   <input type="text" placeholder="Search For" name="store" value="<?php if(isset($_GET['name'])){echo $_GET['name'];} ?>" class="form-control it">
                 </div>
               </div>
 ...
 <div class="card mb-3">
          <div class="card-header">
            ...
            Stores <span class="text-danger"></span></div>
          <div class="card-body">
...
if(isset($_GET['name']) ){
  ...
  $sqls=$conn->query("SELECT * from _security where drank='seller' AND uname LIKE '%$sname%' OR drank='seller' AND dcompany LIKE '%$sname%' OR drank='seller' AND email LIKE '%$sname%' OR drank='seller' AND dphone LIKE '%$sname%' order by id DESC ");
    $total_records =$sqls->num_rows;
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
    $start_from = ($page_no - 1) * $total_records_per_page;

    $site=$conn->query("SELECT * from _security where drank='seller' AND uname LIKE '%$sname%' OR drank='seller' AND dcompany LIKE '%$sname%' OR drank='seller' AND email LIKE '%$sname%' OR drank='seller' AND dphone LIKE '%$sname%' order by id DESC LIMIT $start_from, $total_records_per_page");
}else{  
    $sqls=$conn->query("SELECT * from _security where drank='seller' order by id DESC ");
    $total_records =$sqls->num_rows;
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
    $start_from = ($page_no - 1) * $total_records_per_page;
    
    $site=$conn->query("SELECT * from _security where drank='seller' order by status='online' DESC LIMIT $start_from, $total_records_per_page");
  }
if($site->num_rows>=1){
  while($row=$site->fetch_assoc()){ ?>
    <tr>
<td><?php echo $row['uname']; ?></td>
<td><?php echo $row['dcompany']; ?></td>
<td><?php echo $row['dphone']; ?></td>
<td><?php echo $row['email']; ?></td>
<td><?php echo $row['address']; ?></td>
<td><?php if($row['status']=='offline'){ echo "Active"; }else{ echo "Pending"; } ?></td>
<td>
<div class="btn-group">
<div class="btn-group" >
                    ...
                    <div class="modal-header">
                        ...
                        <span aria-hidden="true">&times;</span>
                             ...
                             <form action="store-process" method="post">
                                    ...
                                    <div class="form-group col-md-12">
										<input type="text" name="store" autocomplete="off" class="form-control" placeholder="Store Name">
									</div>
...
<?php
 $site=$conn->query("SELECT * from _security where drank='seller' order by id DESC ");
 if($site->num_rows>0):
    while($row=$site->fetch_assoc()):?>
      <!-- Modal -->
                    ...
                    <div class="modal-header">
                        ...
                        <span aria-hidden="true">&times;</span>
                             ...
                             <form action="store-process" method="post">
                                    ...
                                    <div class="form-group col-md-12">
										<input type="text" name="store" value="<?php echo $row['dcompany']; ?>" autocomplete="off" class="form-control" placeholder="Store Name">
									</div>
