...
.sweep,.sweep2{
      ...
      <form method="post" action="orders_filter" id="tarp">
               <div class="col-md-7" style="float:left;">
                 <div class="form-group">
                   <input type="text" placeholder="Search For" name="pror_name" value="<?php if(isset($_GET['pro_name'])){echo $_GET['pro_name'];} ?>" class="form-control it">
                 </div>
               </div>
 ...
 <div class="card mb-3">
          <div class="card-header">
            ...
            Manage Returned Orders <span class="text-danger"><?php if(isset($_GET['page_no']) AND @$_GET['page_no'] !=0){ echo "(".$_GET['page_no'].")";} ?></span></div>
          <div class="card-body">
            <div class="table-responsive"  style="min-height:200px">
              <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                ...
                if(isset($_GET['pro_name'])){
                  ...
                  $sqls = $conn->query("SELECT * FROM dcart_holder INNER JOIN login ON dcart_holder.userid=login.userid WHERE dcart_holder.orderid LIKE '%$orderId%' OR login.dname LIKE '%$orderId%' AND dcart_holder.dstatus='returned'  ORDER BY dcart_holder.id DESC");
                $total_records =$sqls->num_rows;
                  $total_no_of_pages = ceil($total_records / $total_records_per_page);
                  $start_from = ($page_no - 1) * $total_records_per_page;
                  
                  $sky =$conn->query("SELECT * FROM dcart_holder INNER JOIN login ON dcart_holder.userid=login.userid WHERE dcart_holder.orderid LIKE '%$orderId%' OR login.dname LIKE '%$orderId%' AND dcart_holder.dstatus='returned'  ORDER BY dcart_holder.id DESC LIMIT $start_from, $total_records_per_page");

                }else{


                $sqls = $conn->query("SELECT * FROM dcart_holder INNER JOIN login ON dcart_holder.userid=login.userid WHERE  dcart_holder.dstatus='returned'  ORDER BY dcart_holder.id ");

                  $total_records =$sqls->num_rows;
                  $total_no_of_pages = ceil($total_records / $total_records_per_page);
                  $start_from = ($page_no - 1) * $total_records_per_page;

                $sky =$conn->query("SELECT * FROM dcart_holder INNER JOIN login ON dcart_holder.userid=login.userid WHERE  dcart_holder.dstatus='returned'  ORDER BY dcart_holder.id LIMIT $start_from, $total_records_per_page");
                }
                if($sky->num_rows>0){
                  while($k=$sky->fetch_assoc()){ ?>
                    <tr>
                      ...
                      <div class="btn-group" >
                          ...
                          <li><a class="nav-link" id="deleter"  orderId="<?php echo $k['orderid']; ?>" href="#">Delete</a></li>
                          <li><a class="nav-link" id="rorders" orderId="<?php echo $k['orderid']; ?>" href="#">Cancel </a></li>
                          </ul>
                      </div>
                      </div>
                      </td>
